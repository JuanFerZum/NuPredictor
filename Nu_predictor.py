...
def run_tomocomd(sdf_file):
    ...
    if not os.path.exists(jar_file):
        ...
        exit()
    cmd_args = ['java', '-jar', 'ToMoCoMD-CARDD_CLI.jar']
    results = sp.run(cmd_args, cwd=tomocomd_path, capture_output=True, text=True)
...
def evaluate_applicability_domain(training_csv, test_csv, model_name, ambit_path):
    ...
    for tech_name, params in techniques.items():
        ...
        '''
        training_base = os.path.basename(training_csv.replace('.csv', '_AmbitInput.csv'))
        test_base = os.path.basename(test_csv.replace('.csv', '_AmbitInput.csv'))
        output_base = f'{model_name}_{tech_name}.csv'
        headers_str = ','.join(standardHeaders)
        cmd_list = ['java', '-jar', 'example-ambit-appdomain-jar-with-dependencies.jar', '-m', str(params['mode']), '-t', training_base, '-s', test_base, '-f', headers_str, '-o', output_base]
        if params.get('extra'):
            cmd_list.extend(params['extra'].split())
        sp.run(cmd_list, cwd=ambit_path)
